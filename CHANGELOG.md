...
按使用截图做的一轮收尾：手机端播放器、收藏页搜索命中、文章底部与搜索结果四处「看着别扭」的地方。

### 变更

- **播放器精修（视觉层级）**：曲目列表的滚动条收成 5px、底色透明、鼠标移到列表上才浮出；曲目之间的分隔线 30% → 18%（不再把列表切成一堆独立卡片）；序号不透明度降到 72%，时间保持右对齐且比曲名弱；上一首 / 下一首提到比三级控件更亮的灰白；播放键的落影收细、按下缩到 0.96；随机按钮的选中底色 14% → 12%；进度条圆点 12px → 14px（hover 1.12 倍），播放中已播时间用强调色；面板顶部那条「当前曲名」从强调色改成中性色 —— 列表里已经高亮过一次，不必再抢一次焦点。
- **当前曲目暂停时显示 ▶**：播放中仍是跳动的音浪条（CSS 动画），暂停时序号位置换成播放小三角；该行同时标上 `aria-current`，屏幕阅读器也能认出正在播的那首。
- **键盘焦点**：上一首 / 下一首 / 播放 / 收起 / 外链 / 进度条 / 音量 / 迷你条补齐 `:focus-visible` 虚线轮廓，沿用站内 `.focus-outline` 那套样式。
- **目录配色**：右侧目录按「节」上色 —— 每开一节色相往前推 60°（蓝 → 紫 → 品红 → 橙 → 黄绿 → 青），一节里的三级标题沿用同一色相、去饱和压淡；左侧色条与阅读进度跟着同一色相走，当前小节加粗 + 同色淡底。文章通篇只有 `###`（没有二级标题）时，每条自己算一节，颜色一样一节一换。
- **手机端播放器**：「顺序 / 随机」与音量各占一半宽度，播放键回到面板中线上（此前被更宽的音量条顶偏）；进度条补上拖动圆点并把轨道调深一档，不再是一条看不出能拖的细线；曲目行更密（行高约 46px → 40px），头部图标两两靠紧，标题与艺术家多留出十几个像素。
- **搜索结果更紧凑**：结果标题 21px → 1rem、摘要 1rem → 0.8125rem 且最多两行，去掉每条结果之间的整宽描边，改用悬停底色；命中的关键词从"褐色方块"改成强调色 + 细下划线；「加载更多结果」跟站内胶囊按钮统一。
- **收藏页命中曲名**：卡片下方那行不再是一串正文大小的裸文本，改成描边胶囊「▶ 曲名」，超长时省略号截断（完整曲名在 title 里）。

### 移除

- **相关阅读**：文章底部不再列出同类文章 —— 栏目、标签、归档、搜索四条路都通了，这块重复且占位置（相关代码与样式一并删掉）。

### 修复

- **自定义滚动条被顶掉**：站内 `html { scrollbar-color: … }` 是继承属性，落到曲目列表上会让 Chrome 改用平台那套约 15px 带箭头的滚动条，`::-webkit-scrollbar` 的尺寸与配色整块失效。现在按 `@supports selector(::-webkit-scrollbar)` 拆成 WebKit 系与 Firefox 两条路，WebKit 一路先把它还原成 `auto`。
- **命中曲名的样式全部失效**：`.album-hit` 是脚本 `createElement` 出来的，拿不到 Astro 的作用域类，样式一条都没生效（这正是"样式丑陋"的根因）。改为 `:global()` 选中后生效，并在原处写明原因。

## 2.2.0 — 2026-09-17

音乐收藏页的一轮大改：播放器从「只挂在收藏页的浮窗」变成**全站常驻**，队列能跨专辑，页面本身补上了搜索与排序。

### 新增

- **常驻播放器**：播放器移到 `Layout` 并带 `transition:persist`，站内翻到任何一页音乐都不断，右下角的迷你条跟着走；离开收藏页不再停播。
- **跨专辑队列**：队列从「专辑 + 曲目双索引」改成扁平的 `{albumId, trackId}[]`。收藏页新增「全部播放」「随机播放全部」，曲目行可「下一首播放」，队列跨专辑时按专辑分组显示。
- **加入队列**：专辑卡右上角的「＋」把这张专辑追加到队尾 —— 不打断当前播放，队列里已有的曲目跳过。
- **Media Session**：锁屏 / 耳机线控 / 系统媒体面板可播放暂停、切歌、拖进度，并显示封面与曲目信息。
- **播放现场恢复**：队列、曲目、进度、播放模式存进 sessionStorage；刷新或从别页进来时迷你条带着上次的位置回来（先不出声，点一下继续）。
- **收藏页检索**：搜索框（专辑 / 艺术家 / 曲名）+ 五种排序（默认 / 专辑名 / 艺术家 / 年份 / 曲目数）；命中曲名时卡片下列出「含《某曲》」，点它直接播那一首。曲库没取到时，按专辑名与艺术家的搜索、以及排序照常可用。
- **失败重试**：播放失败除自动跳下一首外多了「重试」，从上次位置重新加载。
- **标签页标题**：播放时前缀 `▶ 曲名 ·`，切到别的标签也知道在放什么。
- **新专辑**：同步进 3 张（NEGA FRAGMENTS、きらら トリビュート コレクション、Baby Romantica），收藏页现在是 78 张专辑 / 688 首曲目。

### 变更

- **曲库改为按需加载**：新增构建期端点 `/collection/data.json`，播放器与搜索共用一份缓存，点播放时才请求；收藏页 HTML 从 157 KB 降到 90 KB（加上卡片按钮与数据属性后 141 KB）。
- **播放器样式拆分**：独立成 `src/styles/music-player.css`（曲目行由脚本生成，样式必须全局）。
- **收藏页筛选逻辑独立**：`src/utils/collectionFilter.ts` 成为「艺术家 + 关键词 + 排序」的唯一 owner；追番页继续用共用的 `chipFilter`。

### 修复

- **失败提示被覆盖**：`play()` 被拒绝时无条件提示「浏览器拦下了自动播放」，会把音源加载失败的说明连同刚出现的「重试」一起顶掉；现在只在确实没有音源错误时才这么提示。
